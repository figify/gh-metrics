...
async function downloadRepo(client, account, repo, pageSize) {
  try{
    ...
    console.log("Error caught and rethrown in downloadRepo():", error.message )
    throw(err);
  }
};
...
async function downloadTopics(client, account, repo, pageSize, preFetched) {
  try {
    ...
    while(!finished) {
        const { repository } = await client(topicsQuery(account, repo, pageSize, after))
        topics = topics.concat(repository.repositoryTopics.edges.map(edge => edge.node.topic.name));
...
async function downloadIssues(spinner, total, client, account, repo, pageSize) {
  try {
    ...
    while(!finished) {
        const { repository } = await client(issuesQuery(account, repo, pageSize, after))
        let currentIssues = repository.issues.edges.map(edge => edge.node);
        currentIssues = await bluebird.mapSeries(currentIssues, async function(issue, index, arrayLength) {
          // The iteration will be performed sequentially, awaiting for any
          // promises in the process.
          const assignees = await downloadAssignees(client, issue.id, "Issue", pageSize, issue.assignees);
          issue.assignees = assignees;
          return issue;
        });
        currentIssues = await bluebird.mapSeries(currentIssues, async function(issue, index, arrayLength) {
          const labels = await downloadLabels(client, issue.id, "Issue", pageSize, issue.labels);
          issue.labels = labels;
          return issue;
        });
        currentIssues = await bluebird.mapSeries(currentIssues, async function(issue, index, arrayLength) {
          const comments = await downloadIssueComments(client, issue.id, "Issue", pageSize, issue.comments);
...
async function downloadAssignees(client, id, entity, pageSize, preFetched) {
  try {
    ...
    let after = `, after: "${preFetched.pageInfo.endCursor}"`
    while(!finished) {
      const { node } = await client(assigneesQuery(id, entity, pageSize, after))
      assignees = assignees.concat(node.assignees.nodes.map(node => node.login));
    ...
    console.log("Error caught and rethrown in downloadAssignees():", err.message )
    throw(err);
  }
};
...
async function downloadIssueComments(client, id, entity, pageSize, preFetched) {
  try {
    ...
    let after = `, after: "${preFetched.pageInfo.endCursor}"`
    while(!finished) {
      const { node } = await client(issueCommentsQuery(id, entity, pageSize, after))
    ...
    console.log("Error caught and rethrown in downloadIssueComments():", err.message );
    throw(err);
  }
};

async function downloadPullRequests(spinner, total, client, account, repo, pageSize) {
  try {
    let prs = [];
    let finished = false;
    let after = "";
    // TODO(kyrcha): Optimization - Seldect in the query the first assignees and labels since usually the are 0, 1 or 2.
    while(!finished) {
        const { repository } = await client(pullRequestsQuery(account, repo, pageSize, after))
        let currentPRs = repository.pullRequests.edges.map(edge => edge.node);
        currentPRs = await bluebird.mapSeries(currentPRs, async function(pr, index, arrayLength) {
          const assignees = await downloadAssignees(client, pr.id, "PullRequest", pageSize, pr.assignees);
          pr.assignees = assignees;
          return pr;
        });
        currentPRs = await bluebird.mapSeries(currentPRs, async function(pr, index, arrayLength) {
          const labels = await downloadLabels(client, pr.id, "PullRequest", pageSize, pr.labels);
          pr.labels = labels;
          return pr;
        });
        currentPRs = await bluebird.mapSeries(currentPRs, async function(pr, index, arrayLength) {
          const comments = await downloadIssueComments(client, pr.id, "PullRequest", pageSize, pr.comments);
          pr.comments = comments;
          return pr;
        });
        currentPRs = await bluebird.mapSeries(currentPRs, async function(pr, index, arrayLength) {
          const reviews = await downloadPullRequestReviews(client, pr, pageSize, pr.reviews);
          pr.reviews = reviews;
          return pr;
        });
    ...
    console.log("Error caught and rethrown in downloadPullRequests():", err.message )
    throw(err);
  }
};
...
async function downloadPullRequestReviews(client, pr, pageSize, preFetched) {
  try {
    ...
    let after = `, after: "${preFetched.pageInfo.endCursor}"`
    while(!finished) {
      const { node } = await client(pullRequestsReviewsQuery(pr.id, pageSize, after))
      let currentPRReviews = node.reviews.edges.map(edge => edge.node);
      currentPRReviews = await bluebird.mapSeries(currentPRReviews, async function(review, index, arrayLength) {
        const comments = await downloadPullRequestReviewThreads(client, pr.id, pageSize, pr.reviewThreads); // need the id of the PR not the PR review
    ...
    console.log("Error caught and rethrown in downloadPullRequestReviews():", err.message )
    throw(err);
  }
};
...
async function downloadPullRequestReviewThreads(client, id, pageSize, preFetched) {
  try {
    ...
    let after = `, after: "${preFetched.pageInfo.endCursor}"`
    while(!finished) {
      const { node } = await client(pullRequestsReviewThreadsQuery(id, pageSize, after))
      let currentPRThreads = node.reviewThreads.edges.map(edge => edge.node);
      currentPRThreads = await bluebird.mapSeries(currentPRThreads, async function(thread, index, arrayLength) {
        const comments = await downloadPullRequestReviewComments(client, thread.id, pageSize, thread.comments);
    ...
    return _.chain(threads).map(thread => thread.comments).flatten().value();
  } catch(err) {
    console.log("Error caught and rethrown in downloadPullRequestReviewThreads():", err.message )
    throw(err);
  }
};

async function downloadPullRequestReviewComments(client, threadId, pageSize, preFetched) {
  try {
    let comments = preFetched.edges.map(edge => edge.node);
    let finished = !preFetched.pageInfo.hasNextPage;
    let after = `, after: "${preFetched.pageInfo.endCursor}"`
    while(!finished) {
      const { node } = await client(pullRequestsReviewCommentsQuery(threadId, pageSize, after))
      comments = comments.concat(node.comments.edges.map(edge => edge.node));
      if(!node.comments.hasNextPage) {
          finished = true;
      } else {
    ...
    console.log("Error caught and rethrown in downloadPullRequestReviewComments():", err.message )
    throw(err);
  }
};
